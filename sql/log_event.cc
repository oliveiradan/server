...
void Start_log_event_v3::print(FILE* file, PRINT_EVENT_INFO* print_event_info)
{
      ...
      !print_event_info->short_form)
  {
    ...
    print_base64(&cache, print_event_info, do_print_decoded_base64);
...
/*
  ...
  event string. In case the event ends a statement the encoded event's
  cache is copied into the argument file. Copying considers how to
  wrap the event. When the encoded data size is within max(MAX_ALLOWED_PACKET)
  a regular BINLOG query is composed. Otherwise it is build as fragmented

    BINLOG number_of_fragments,'user_var_name_prefix'

  where fragments are represented by an "indexed" user variable. E.g
  when the above variable's name prefix is 'binlog_fragment' and the
  number of fragments is 2 the fragmented version is as the following:
