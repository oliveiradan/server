  ...
  */
  ...
  if (thd->lex->fragmented_binlog_event > 0)
  {
    /*
      ...
      memory allocation.  The resulted size can exceed
      max(max_allowed_packet) which is not a problem while String
      class methods are not used.
      Compute decoded_len of the merged fragments.
    */
    const char *name_fmt= "%s_%d";
    name_buf= (char *) my_malloc(thd->lex->ident.length + 1 +
                                       (sizeof(thd->lex->
    ...
    for (uint i= 0; i < thd->lex->fragmented_binlog_event; i++)
    {
      ...
      {
        /*
          Todo: specify the exact reason in 10.4 error message:
          BINLOG fragment user variable 'name' does not hold
          appropriate value.
        */
