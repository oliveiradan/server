...
/**
  ...
  SYNOPSIS
    ...
    Other arguments represent format strings to print with
                       ...
                       char* buf)
{
  ...
  for (i= 0, written_off_last_block= 0;
       i < n_frag;
       i++, total_written += frag_written)
  {
       ...
       {
            /*
              Either the current block is the last (L) and possible
              has some extra not to fit (LG) into the fragment, or (I)
              the current (whole then) block is intermediate.
            */
            ...
            if (frag_written == frag_size) // (L) condtion
            {
